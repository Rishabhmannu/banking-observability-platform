...
echo "🚀 Safe System Restart v6.1 - Complete AIOps Platform with Kubernetes Monitoring"
echo "================================================================================="
...
# NEW: Check Kubernetes cluster health
...
if [ -d "backups" ]; then
    ...
    if [ ${#backups[@]} -eq 0 ]; then
        ...
        if [[ "$backup_choice" =~ ^[0-9]+$ ]] && [ "$backup_choice" -ge 1 ] && [ "$backup_choice" -le ${#backups[@]} ]; then
            ...
            for dir in prometheus grafana redis container-resource-monitor redis-cache-analyzer kubernetes-monitoring; do
...
echo "   (1/9) 🔧 Starting core infrastructure (Prometheus, Grafana, cAdvisor)..."
...
echo "   (2/9) 🗄️ Starting all databases (MySQL, Postgres, Redis)..."
...
echo "   (3/9) 📨 Starting message brokers (RabbitMQ, Kafka)..."
...
echo "   (4/9) 📈 Starting data exporters and core monitors..."
...
echo "   (5/9) 🔍 Starting Jaeger tracing..."
docker compose "${COMPOSE_FILES[@]}" up -d jaeger
sleep 20

# Phase 6: Application Services (Banking, ML, Analyzers)
echo "   (6/9) 🏦 Starting all application services..."
...
# Phase 7: API Gateway (starts after backend services are up)
echo "   (7/9) 🌐 Starting API gateway..."
docker compose "${COMPOSE_FILES[@]}" up -d api-gateway
sleep 20

# Phase 8: Generators (start last as they depend on the whole system)
echo "   (8/9) ⚡ Starting all generators..."
docker compose "${COMPOSE_FILES[@]}" up -d load-generator trace-generator cache-load-generator resource-anomaly-generator
sleep 15

# NEW: Phase 9: Kubernetes Monitoring Services
echo "   (9/9) ☸️  Starting Kubernetes monitoring services..."
if kubectl get nodes >/dev/null 2>&1; then
...
# NEW: Add Kubernetes service check
if kubectl get nodes >/dev/null 2>&1; then
