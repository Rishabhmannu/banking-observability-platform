...
echo "🚀 Safe System Restart v6 - Complete AIOps Platform with Full Dependency Management"
echo "=================================================================================="
...
if [ -d "backups" ]; then
    ...
    if [ ${#backups[@]} -eq 0 ]; then
        ...
        if [[ "$backup_choice" =~ ^[0-9]+$ ]] && [ "$backup_choice" -ge 1 ] && [ "$backup_choice" -le ${#backups[@]} ]; then
            ...
            for dir in prometheus grafana redis container-resource-monitor redis-cache-analyzer; do
...
echo "   (1/8) 🔧 Starting core infrastructure (Prometheus, Grafana, cAdvisor)..."
...
echo "   (2/8) 🗄️ Starting all databases (MySQL, Postgres, Redis)..."
...
echo "   (3/8) 📨 Starting message brokers (RabbitMQ, Kafka)..."
...
echo "   (4/8) 📈 Starting data exporters and core monitors..."
...
echo "   (5/8) 🔍 Starting Jaeger tracing..."
docker compose "${COMPOSE_FILES[@]}" up -d jaeger
sleep 20

# Phase 6: Application Services (Banking, ML, Analyzers)
# FIXED: Added anomaly-injector to this phase
echo "   (6/8) 🏦 Starting all application services..."
...
echo "   (8/8) ⚡ Starting all generators..."
...
services_to_check=(
    ...
    "Cache Load Generator:http://localhost:5013/status"
