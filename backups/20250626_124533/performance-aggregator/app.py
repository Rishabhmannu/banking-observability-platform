...
class MetricsAggregator:
    ...
    def initialize_aggregator_metrics(self):
        """Initialize all metrics with default values"""
        # Initialize SLO compliance metrics
        slo_compliance_percentage.labels(slo_type='response_time_p95').set(0)
        slo_compliance_percentage.labels(slo_type='error_rate').set(100)
        slo_compliance_percentage.labels(slo_type='availability').set(100)

        # Initialize business hours metrics
        business_hour_transaction_rate.set(0)
        off_hour_transaction_rate.set(0)

        # Initialize performance scores
        for category in ['latency', 'reliability', 'throughput', 'overall']:
            transaction_performance_score.labels(category=category).set(0)

        logger.info("Aggregator metrics initialized")
    ...
    def calculate_business_hours_metrics(self):
        ...
        try:
            ...
            if result and result.get('status') == 'success' and result['data']['result']:
                ...
                if len(result['data']['result']) > 0 and 'values' in result['data']['result'][0]:
                    values = result['data']['result'][0]['values']
                    if values and len(values) > 0:
                        rates = [float(v[1]) for _, v in values if v[1] != 'NaN']

                        if rates:
                            current_rate = rates[-1]
                            if is_business_hour:
                                business_hour_transaction_rate.set(current_rate)
                                # Also set off_hour to 0 if not set
                                if off_hour_transaction_rate._value.get() == 0:
                                    off_hour_transaction_rate.set(0)
                            else:
                                off_hour_transaction_rate.set(current_rate)
                                # Also set business_hour to 0 if not set
                                if business_hour_transaction_rate._value.get() == 0:
                                    business_hour_transaction_rate.set(0)
                            logger.info(
                                f"Set {'business' if is_business_hour else 'off'} hour rate: {current_rate}")
                        else:
                            logger.warning("No valid rate values found")
                    else:
                        logger.warning("No values in result")
                else:
                    logger.warning("Empty result from Prometheus")
            else:
                logger.warning(
                    "Failed to query Prometheus for business hours metrics")
                # Set default values
                if is_business_hour:
                    business_hour_transaction_rate.set(0)
                else:
            ...
            logger.error(f"Error in calculate_business_hours_metrics: {e}")
            # Set safe defaults
            business_hour_transaction_rate.set(0)
            off_hour_transaction_rate.set(0)
    ...
    def calculate_slo_compliance(self):
        ...
        try:
            ...
            if p95_result and p95_result.get('status') == 'success' and p95_result['data']['result']:
                if len(p95_result['data']['result']) > 0 and 'values' in p95_result['data']['result'][0]:
                    ...
                    if values and len(values) > 0:
                        ...
                        if p95_values:
                            ...
                            compliance_pct = (compliant_count /
                                            len(p95_values)) * 100
                            slo_compliance_percentage.labels(
                                slo_type='response_time_p95').set(compliance_pct)
                            logger.info(
                                f"Response time SLO compliance: {compliance_pct:.2f}%")
                        else:
                            slo_compliance_percentage.labels(
                                slo_type='response_time_p95').set(0)
                    else:
                        slo_compliance_percentage.labels(
                            slo_type='response_time_p95').set(0)
                else:
            ...
            if error_result and error_result.get('status') == 'success' and error_result['data']['result']:
                if len(error_result['data']['result']) > 0 and 'values' in error_result['data']['result'][0]:
                    ...
                    if values and len(values) > 0:
                        ...
                        if error_rates:
                            ...
                            logger.info(
                                f"Error rate SLO compliance: {compliance_pct:.2f}%")
                        else:
                            # If no errors, that's 100% compliance
                            slo_compliance_percentage.labels(
                                slo_type='error_rate').set(100)
                    else:
                        slo_compliance_percentage.labels(
                            slo_type='error_rate').set(100)
                else:
                    # No data means no errors, so 100% compliance
                    slo_compliance_percentage.labels(
                        slo_type='error_rate').set(100)
            else:
                slo_compliance_percentage.labels(slo_type='error_rate').set(100)
            ...
            if availability_result and availability_result.get('status') == 'success':
                ...
                for service in availability_result['data']['result']:
                    if 'values' in service and service['values']:
                        service_avail = []
                        for _, v in service['values']:
                            try:
                                if v != 'NaN' and v != 'null':
                                    service_avail.append(float(v))
                            except (ValueError, TypeError):
                                continue
                        if service_avail:
                            avg_availability = np.mean(service_avail)
                ...
                if availabilities:
                    ...
                    logger.info(
                        f"Availability SLO compliance: {compliance_pct:.2f}%")
                else:
                    slo_compliance_percentage.labels(
                        slo_type='availability').set(100)
            else:
                slo_compliance_percentage.labels(slo_type='availability').set(100)
            ...
            logger.error(f"Error in calculate_slo_compliance: {e}")
            # Set default values for all SLO types
            slo_compliance_percentage.labels(slo_type='response_time_p95').set(0)
            slo_compliance_percentage.labels(slo_type='error_rate').set(100)
            slo_compliance_percentage.labels(slo_type='availability').set(100)
...
if __name__ == '__main__':
    # Initialize metrics
    aggregator.initialize_aggregator_metrics()
