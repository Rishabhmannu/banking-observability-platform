...
cp docker-compose.transaction-monitoring.yml "$backup_dir/" 2>/dev/null && echo "   ✅ docker-compose.transaction-monitoring.yml backed up"
...
# Save dashboard URLs for reference
echo "📝 Saving dashboard URLs..."
cat > "$backup_dir/dashboard_urls.txt" << EOF
Dashboard URLs after restart:
- Banking Overview: http://localhost:3000/d/banking-overview
- DDoS Detection: http://localhost:3000/d/ddos-detection  
- Auto-Baselining: http://localhost:3000/d/auto-baselining
- Transaction Performance: http://localhost:3000/d/transaction-performance
EOF
...
for service in "${services_to_stop[@]}"; do
    ...
    if docker compose -f docker-compose.yml -f docker-compose.transaction-monitoring.yml stop "$service" >/dev/null 2>&1; then
        echo -e "${GREEN}✅ Stopped${NC}"
    else
        # Try without the transaction monitoring compose file
