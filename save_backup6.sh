...
echo "💾 Save Backup v5.0 - Create Full System Backup with Correlation & RCA Services (No Shutdown)"
echo "=========================================================================================="
...
# NEW: Check Correlation & RCA services
echo -n "📊 Event Correlation Engine (5025): "
if curl -s --connect-timeout 2 http://localhost:5025/health >/dev/null 2>&1; then
...
if curl -s --connect-timeout 2 http://localhost:5026/health >/dev/null 2>&1; then
    echo -e "${GREEN}✅ RUNNING${NC}"
else
    echo -e "${YELLOW}⚠️  OFFLINE${NC}"
fi
...
if curl -s http://localhost:3000/api/health | grep -q "ok"; then
    ...
    echo "   This preserves your manual edits including correlation dashboards!"
    ...
    if [ ! -z "$dashboards" ]; then
        ...
        echo "$dashboards" | jq -r '.[] | .uid + ":" + .title' | while IFS=: read -r uid title; do
            ...
            if [ ! -z "$dashboard_json" ]; then
                ...
                if echo "$title" | grep -qE -i "(kubernetes|k8s|pod|node|hpa|autoscal)"; then
                    echo -e "${CYAN}✅ K8s${NC}"
                    ((k8s_dashboards++))
                # Check if this is a Correlation/RCA dashboard
                elif echo "$title" | grep -qE -i "(correlation|rca|insights|analysis)"; then
                    echo -e "${PURPLE}✅ RCA${NC}"
                    ((correlation_dashboards++))
                else
                    echo -e "${GREEN}✅${NC}"
                fi
                ((dashboard_count++))
        ...
        echo "   🔗 Correlation/RCA dashboards: $correlation_dashboards"
...
# NEW: Backup Correlation & RCA Engines
...
if [ "$running_containers" -gt 0 ]; then
    echo -n "   Capturing metrics state... "
    {
        ...
        # NEW: Correlation & RCA Engine Status
...
# System Backup with Correlation & RCA Engines - $backup_timestamp
Generated while system was $([ "$running_containers" -gt 0 ] && echo "RUNNING" || echo "STOPPED")

## Quick Restore
Run: \`./safe_restart8.sh\` and select this backup when prompted
...
- **Event Correlation Engine** configurations and state
- **RCA Insights Engine** configurations and OpenAI setup
- **Correlation RCA Dashboard** (Streamlit application)
- **Complete Kubernetes Monitoring System**
...
$([ "$running_containers" -gt 0 ] && echo "- Live metrics snapshot with correlation & RCA status")
...
- **Event Correlation Engine (Port 5025)**
  - Statistical correlation analysis
  - Prometheus metric collection
  - Time-window pattern detection
  - Correlation confidence scoring
- **RCA Insights Engine (Port 5026)**
  - AI-powered root cause analysis
  - OpenAI GPT integration
  - Business impact assessment
  - Remediation recommendations
- **Correlation RCA Dashboard (Port 8501)**
...
- **Dashboards**: Kubernetes Overview, Pod Auto-scaling, Node Consumption, Correlation Analysis
...
- **Event Correlation Engine (5025)**
- **RCA Insights Engine (5026)**
- **Correlation RCA Dashboard (8501)**
- **Kubernetes Monitoring System (9419)**
- **Pod Auto-scaling with HPA**
- **K8s Resource Monitor and Dashboards**

## To Use This Backup
1. Run: ./safe_restart8.sh
2. When prompted for backup selection, choose: $backup_timestamp
3. The system will restore all configurations and dashboards
4. Correlation & RCA engines will be automatically started
5. Kubernetes services will be automatically deployed
...
- Correlation & RCA engine configurations and OpenAI setup preserved
...
- PDF report generation capabilities included
EOF
...
echo "🔄 To restore from this backup: ./safe_restart8.sh"
echo ""
...
echo "  • Preserve all correlation & RCA configurations"
echo "  • Restore Kubernetes monitoring with all dashboards and automation"
echo ""
echo -e "${CYAN}🔗 New Correlation & RCA Features Backed Up:${NC}"
echo "  • Event correlation analysis with statistical significance"
echo "  • AI-powered root cause analysis with OpenAI integration"
echo "  • Professional PDF report generation"
echo "  • Interactive Streamlit dashboard for correlation exploration"
echo "  • Comprehensive system monitoring and alerting"
echo "  • Business impact assessment and remediation recommendations"